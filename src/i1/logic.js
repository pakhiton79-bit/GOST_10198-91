...
// планки (PLANK_WIDTH=100мм, см. w:100 у "Планка"/"Доска..." в calc.js)
...
// каждая = wallValue) - не фиксированное значение отступа, а МИНИМУМ (по
// уточнению пользователя: раньше отступ был жёстко зафиксирован на
// wallValue*2 и не менялся вместе с числом/шагом поясов - это и было
// ошибкой). Если при заданном count равномерный отступ выходит меньше
// минимума (или вовсе отрицательный - планки не помещаются) - мест для
// такого числа поясов недостаточно (см. calculate() - жёсткий блок, как и
// при boardLen/6 < 0 у штатного правила). Само число поясов - либо
// берётся как есть (override.mode==='count', обязательно целое, минимум
