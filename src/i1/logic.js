...
// значение отступа, а МИНИМУМ (по уточнению пользователя: раньше отступ
// был жёстко зафиксирован на wallValue*2 и не менялся вместе с числом/
// шагом поясов - это и было ошибкой). Если при заданном count равномерный
// шаг выходит меньше минимума - мест для такого числа поясов недостаточно
// (см. calculate() - жёсткий блок, как и при boardLen/6 < 0 у штатного
// правила). Само число поясов - либо берётся как есть (override.mode===
// 'count', обязательно целое, минимум 2), либо выбирается максимальным
// таким, чтобы равномерный шаг не превышал заданного пользователем значения
...
function plankCount(boardLen, wallValue, override){
  if(override){
    ...
    const count = override.mode === 'count'
      ...
      : Math.max(2, ceilInt(boardLen/override.value)-1);
    ...
    // (штатное правило boardLen/6 тоже даёт дробные мм).
