...
// ДОЛИ от размеров самой рамки щита на фото (Wp=R-L, Hp=B-T; L/R/T/B -
// наружные левый/правый/верхний/нижний края щита) - НЕ как множители
// ширины стойки u, как было в первой версии: у разных фото отношение
// u/IW (ширина стойки к общей ширине снимка) сильно отличается (у фото с
// бОльшим числом стоек она меньше, т.к. стойки на фото уже) - при
// масштабировании готового чертежа на экране/печати до фиксированной
// ширины (~260px, см. renderDiagram) это давало непропорционально
// маленькие/большие отступы у скобок на разных фото и "кривой" результат
// (по прямому репорту пользователя). Сама же рамка щита занимает на всех
// присланных фото примерно одинаковую долю кадра, поэтому доли от Wp/Hp
// дают одинаковые ПРОПОРЦИИ чертежа независимо от фото. Формула одна для
// всех схем (2/3/4 стойки = 1/2/3 раскосины) - меняются только сами
// L/R/T/B конкретного фото (измерены отдельно по каждому файлу).
function torecRecords(L, R, T, B){
  const Wp = R - L, Hp = B - T;
  function px(fx){ return L + fx*Wp; }
  function py(fy){ return T + fy*Hp; }
  return function(longbeamVal, widthVal, skinVal, heightVal){
    ...
    if(longbeamVal > 0){
      records.push(
        {type:'line', x1:px(0.855), y1:py(0), x2:px(1.305), y2:py(0)},
        {type:'line', x1:px(0.853), y1:py(-0.109), x2:px(1.203), y2:py(-0.109)},
        {type:'line', x1:px(1.159), y1:py(-0.109), x2:px(1.159), y2:py(0)},
        {type:'single', x1:px(1.0), y1:py(-0.330), x2:px(1.159), y2:py(-0.058), lx:px(0.960), ly:py(-0.364), text:longbeamVal+' мм'}
      );
    }
    ...
    records.push(
      {type:'line', x1:px(0), y1:py(1.0), x2:px(0), y2:py(1.362)},
      {type:'line', x1:px(1.0), y1:py(1.0), x2:px(1.0), y2:py(1.382)},
      {type:'double', x1:px(1.0), y1:py(1.307), x2:px(0), y2:py(1.307), lx:px(0.504), ly:py(1.340), text:widthVal+' мм'}
    );
    // Группа C - толщина досок обшивки бока (skin.value) - небольшой
    // отступ слева от щита.
    records.push(
      {type:'line', x1:px(-0.106), y1:py(1.0), x2:px(-0.106), y2:py(1.270)},
      {type:'line', x1:px(-0.106), y1:py(1.207), x2:px(0), y2:py(1.207)},
      {type:'single', x1:px(-0.219), y1:py(1.369), x2:px(-0.050), y2:py(1.207), lx:px(-0.222), ly:py(1.403), text:skinVal+' мм'}
    );
    // Группа D - высота щита БЕЗ продольного бруса крышки (ширина
    // стойки*2 + длина стойки) = 100*2 + torecFrame.len.
    records.push(
      {type:'line', x1:px(0.904), y1:py(1.143), x2:px(1.298), y2:py(1.140)},
      {type:'double', x1:px(1.239), y1:py(0), x2:px(1.239), y2:py(1.142), lx:px(1.251), ly:py(0.568), text:heightVal+' мм', vertical:true}
    );
    return records;
  };
}
...
// Натуральные размеры фото и измеренные границы каркаса (L/R/T/B) - см.
// комментарий у torecRecords выше. Разметка для «2 стойки» - по фото,
// прямо размеченному пользователем; для «3/4 стойки» - границы рамки щита
// (L/R/T/B) измерены тем же способом (сетка координат по фото), а сами
// подписи вычисляются той же функцией torecRecords (доли от Wp/Hp) -
// первая попытка (доли от ширины стойки u) дала на этих фото "кривой"
// результат, см. комментарий у torecRecords выше.
const TOREC_VARIANTS = {
  2: { img: TOREC_IMG_2POSTS_B64, IW: 1116, IH: 796, records: torecRecords(107.5, 1009.5, 79.5, 696.5) },
  3: { img: TOREC_IMG_3POSTS_B64, IW: 1460, IH: 605, records: torecRecords(81.5, 1377.5, 63.5, 596.5) },
  4: { img: TOREC_IMG_4POSTS_B64, IW: 2222, IH: 644, records: torecRecords(77.5, 2135.5, 63.5, 635.5) },
};
