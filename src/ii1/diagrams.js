...
// Крышка, Щит торцевой, Щит боковой и Дно - готовы (см.
// diagramKryshka()/diagramTorec()/diagramBok()/diagramDno() ниже): геометрия
// (координаты стрелок и подписей в натуральных пикселях фото) - из присланных
// заказчиком схем.
...
const KRYSHKA_0L_4P_IMG_B64 = "data:image/jpeg;base64,__IMG:kryshka_0l_4p.jpg__";
const KRYSHKA_2L_2P_IMG_B64 = "data:image/jpeg;base64,__IMG:kryshka_2l_2p.jpg__";
const KRYSHKA_2L_3P_IMG_B64 = "data:image/jpeg;base64,__IMG:kryshka_2l_3p.jpg__";
const KRYSHKA_3L_2P_IMG_B64 = "data:image/jpeg;base64,__IMG:kryshka_3l_2p.jpg__";
const KRYSHKA_3L_3P_IMG_B64 = "data:image/jpeg;base64,__IMG:kryshka_3l_3p.jpg__";
const KRYSHKA_3L_4P_IMG_B64 = "data:image/jpeg;base64,__IMG:kryshka_3l_4p.jpg__";
const KRYSHKA_4L_4P_IMG_B64 = "data:image/jpeg;base64,__IMG:kryshka_4l_4p.jpg__";
...
// --- Щит торцевой ---
// Геометрия подписей - НЕ единая формула на все схемы (более ранняя
// версия, выводившая координаты 3/4-стоечных схем как доли от размеров
// рамки щита по разметке 2-стоечной схемы, давала на некоторых схемах
// заметный перекос - см. историю git), а собственная разметка КАЖДОЙ
// схемы, присланная пользователем отдельно для каждого фото (тот же
// принцип, что и у KRYSHKA_VARIANTS выше - независимые записи per-схема,
// в натуральных пикселях именно этого фото). Схемы сгруппированы по
// этажности (TOREC_VARIANTS[floors][count]) - геометрия/пропорции у
// 1-этажных и 2-этажных щитов принципиально разные (два яруса раскосин),
// поэтому при floors=2 fallback ищет ближайшее число стоек ТОЛЬКО среди
// готовых 2-этажных схем, не подменяя их 1-этажным фото (см.
// nearestTorecVariant ниже).
...
const TOREC_IMG_2FLOOR_2POSTS_B64 = "data:image/jpeg;base64,__IMG:torec_ii1_2floor_2posts.jpg__"; // 2 этажа, 2 раскосины
const TOREC_IMG_2FLOOR_3POSTS_B64 = "data:image/jpeg;base64,__IMG:torec_ii1_2floor_3posts.jpg__"; // 2 этажа, 4 раскосины
const TOREC_IMG_2FLOOR_4POSTS_B64 = "data:image/jpeg;base64,__IMG:torec_ii1_2floor_4posts.jpg__"; // 2 этажа, 6 раскосин

const TOREC_VARIANTS = {
  1: {
    // «2 стойки» (1 раскосина) - по исходной разметке пользователя. Группа D
    // (нижняя засечка/стрелка) в исходных данных заканчивалась чуть ниже
    // фактического края щита (y≈784 вместо B=696.5 - видимо, неточность в
    // самой первой, ещё не выверенной разметке) - выровнено по чистому B/T,
    // по образцу уже уточнённых схем «3/4 стойки» ниже (там обе засечки
    // группы D ровно на T и на B).
    ...
    // «3 стойки» (2 раскосины) - по разметке пользователя. Левый конец
    // стрелки widthVal и низ ближней (x=82) вертикальной линии группы
    // skinVal - подправлены по аналогии с исправлением у «4 стойки» ниже
    // (тот же тип ошибки: стрелка ширины начиналась у дальней линии x=10,
    // а должна - у ближней x=82, вплотную к видимому краю стойки на фото;
    // ближняя линия соответственно удлинена, чтобы визуально доходить до
    // стрелки) - у «2 стойки» выше эта же пара элементов уже была верной
    // (там своя отдельная линия-вынос для ширины, не переиспользует линию
    // группы skinVal).
...
// при нынешнем наборе фото, floors всегда 1 или 2 - см. buildFrame в
// calc.js) - используем схемы доступной этажности (тот же приём, что и у
// Крышки, nearestKryshkaVariant выше, но с дополнительным измерением
// "этаж").
...
// --- Щит боковой ---
// Та же схема группировки, что и у Щита торцевого (TOREC_VARIANTS выше):
// собственная разметка КАЖДОЙ схемы, сгруппированная по этажности
// (BOK_VARIANTS[floors][count]). Группы подписей на 1-этажных схемах:
// - A (только если longbeamVal>0 - режим "поперечное" расположение досок
...
const BOK_IMG_1FLOOR_2POSTS_B64 = "data:image/jpeg;base64,__IMG:bok_ii1_1floor_2posts.jpg__"; // 1 раскосина
const BOK_IMG_1FLOOR_3POSTS_B64 = "data:image/jpeg;base64,__IMG:bok_ii1_1floor_3posts.jpg__"; // 2 раскосины
const BOK_IMG_1FLOOR_4POSTS_B64 = "data:image/jpeg;base64,__IMG:bok_ii1_1floor_4posts.jpg__"; // 3 раскосины
const BOK_IMG_2FLOOR_2POSTS_B64 = "data:image/jpeg;base64,__IMG:bok_ii1_2floor_2posts.jpg__"; // 2 этажа, 2 раскосины
const BOK_IMG_2FLOOR_3POSTS_B64 = "data:image/jpeg;base64,__IMG:bok_ii1_2floor_3posts.jpg__"; // 2 этажа, 4 раскосины
const BOK_IMG_2FLOOR_4POSTS_B64 = "data:image/jpeg;base64,__IMG:bok_ii1_2floor_4posts.jpg__"; // 2 этажа, 6 раскосин
...
const BOK_VARIANTS = {
  1: {
    ...
    // «4 стойки» (3 раскосины) - геометрия снята с самого фото (панель без
    // готовой пользовательской разметки, замерена по пиксельным границам
    // рамки/стоек), группы длины/высоты/продольного бруса расставлены по
    // аналогии с остальными схемами. Фото 2220x646 почти совпадает по
    // размеру с TOREC_VARIANTS[1][4] (2222x644, тот же класс "4 стойки
    // растянутые на всю ширину") - для групп высоты и продольного бруса
    // использованы те же координаты (масштаб идентичен), для длины - свои,
    // по фактическим краям этого фото (x≈6 и x≈2211).
  ...
  2: {
    ...
    // полная высота щита (heightVal), поэтому records() её не использует.
    ...
    // краям этого фото (замерено по пиксельным границам, x≈6 и x≈1446), группы
    // высоты/продольного бруса взяты из TOREC_VARIANTS[2][3]
    // (torec_ii1_2floor_3posts.jpg, 1473x1088 - тот же масштаб, что и у этого
    // фото 1455x1068, с точностью до ~2%) - см. тот же приём, что и у
    // BOK_VARIANTS[1][4] выше.
    ...
    // разметки для этого фото. Группа длины - свои координаты по фактическим
    // краям этого фото (x≈6 и x≈2211), группы высоты/продольного бруса взяты
    // из TOREC_VARIANTS[2][4] (torec_ii1_2floor_4posts.jpg, 2223x1160 -
    // практически идентичный масштаб этому фото 2220x1160).
...
// Тот же приём, что и у nearestTorecVariant выше: сперва ближайшая доступная
