...
function computeGost10198II1(input){
  ...
  // Вспомогательная функция: считает число секций (стоек) по каркасу щита -
  // минимум по шагу осей ≤800мм (docx: "расстояние между осями не более
  // 800мм"), при необходимости растим дальше, если угол раскосины <20°
  // (аналогично типу I-3, torecSectionWidth/torecAngleDeg в src/app.js) -
  // растим, пока это физически возможно (ширина секции ещё положительна).
  function buildFrame(fillspace, panelH){
    let count = Math.max(2, ceilInt(fillspace/800) + 1);
    function sectionW(n){ return fillspace/(n-1); }
    ...
    let warn = null;
    if(len > 0){
      while(angleDeg(count, len) < 20 && sectionW(count+1) > 0){
        count++;
      }
    ...
    return {count, floors, len, sectionW: sectionW(count), hasRaskosina, warn};
  }
