...
*/
// Логотип водяного знака (см. print-watermark ниже) - раньше был буквой «F»,
// потом заменён на «Г» по фото, загруженному пользователем; letter-логотип
// вырезан из этого фото (белая буква на сплошном фоне) и перекрашен в цвет
// бренда с прозрачным фоном - тем же способом, что и старый логотип «F».
// Как и остальные картинки проекта - через __IMG:, не как отдельный
// захардкоженный base64 (см. IMG_PLACEHOLDER в build.py).
const LOGO_B64 = "data:image/png;base64,__IMG:logo_watermark.png__";
...
function reserveDiagramOverflow(printArea){
  printArea.querySelectorAll('.diagram-slot').forEach(slot=>{
    ...
    // Центрируем весь видимый охват чертежа (картинка + вылетающие подписи/
    // стрелки) в слоте, а не просто прижимаем его вылет к левому краю - иначе
    // у разных узлов (разная ширина, разный вылет) при печати друг под другом
    // получалась "лесенка", а не ровный столбец (см. правку пользователя).
    // Остаток свободного места (slack) делим пополам между левым и правым
    // отступом - тот же приём, что и в reserveDiagramOverflowScreen выше.
    const m = measure();
    const leftGap = Math.max(0, Math.ceil(m.box.left - m.left));
    const rightGap = Math.max(0, Math.ceil(m.right - m.box.right));
    const footprint = leftGap + m.box.width + rightGap;
    const slack = Math.max(0, slotWidth - footprint);
    wrap.style.marginLeft = (leftGap + slack / 2) + 'px';
  });
}
...
function reserveDiagramOverflowScreen(container){
  container.querySelectorAll('.diagram-slot').forEach(slot=>{
    ...
    // До этой правки чертёж просто прижимался левым краем видимого охвата
    // (картинка + вылетающие подписи/стрелки) к левому краю слота - у разных
    // узлов разная ширина и разный вылет, поэтому при просмотре друг под
    // другом (Дно/Крышка/Торец/Бок) получалась "лесенка", а не ровный
    // столбец. Вместо этого центрируем весь видимый охват в фиксированном по
    // ширине слоте - остаток свободного места (slack) делим пополам между
    // левым и правым отступом.
    const footprint = leftGap + m.box.width + rightGap;
    const slack = Math.max(0, DIAGRAM_SLOT_BUDGET - footprint);
    const centerExtra = slack / 2;

    wrap.style.marginTop = topGap + 'px';
    wrap.style.marginBottom = bottomGap + 'px';
    wrap.style.marginLeft = (leftGap + centerExtra) + 'px';
  });
}
