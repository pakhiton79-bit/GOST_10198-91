...
function computeGost10198I3(input){
  const {L, W, H, MASS, optimizeSizes, removeFloorBoards, removeSkidBoards,
         roundBoardWidths, solidRigidBase, forkliftLoading} = input;
  ...
  if(solidRigidBase){
    ...
    if(sel.spacingExceeded){
      // sel.count здесь - НЕ значение из самой Табл.19 (в ней для лёгких масс
      // максимум 3, для самых тяжёлых - 5), а количество, достроенное сверх
      // таблицы по её же примечанию (ширина сечения снижается на 1 градацию на
      // каждый дополнительный полоз, см. extrapolatedCount выше) вплоть до
      // предела градации сечений (SKID_GRADATIONS, минимум 40мм) - дальше
      // увеличивать количество уже нельзя, а требование ≤1200мм всё ещё не
      // выполнено. Формулировка не должна создавать впечатление, что это
      // количество взято прямо из таблицы.
      warnings.push(`Шаг между осями полозьев >1200 мм (п.1.6.2) не устранён — количество полозьев достроено сверх Табл. 19 (по её примечанию: ширина сечения снижается на 1 градацию на каждый дополнительный полоз) до предела градации сечений (${sel.w} мм) — принято максимально возможное количество (${sel.count} шт.), дальше увеличивать нельзя.`);
    }
