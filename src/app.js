...
function computeGost10198I3(input){
  ...
  if(solidRigidBase){
    ...
    if(sel.spacingExceeded){
      // Короткая формулировка по просьбе пользователя - подробности (что
      // sel.count не из самой Табл.19, а достроено сверх неё по её же
      // примечанию вплоть до предела градации сечений) см. в истории
      // изменений/предыдущей версии этого сообщения.
  ...
  // Верхнего предела по наружной высоте нет (искусственный потолок в 4000мм
  // убран по уточнению пользователя) - свыше 2000мм расчёт продолжается по
  // той же схеме 2 этажей, размеры деталей растут линейно вместе с высотой.
  ...
  // а не по наружной высоте ящика outerH - в отличие от остального (переход на
  // 2 этажа при outerH>2000мм и т.п.), которое остаётся по outerH (по
