...
let requiredThicknessNotAvailable = false; // взводится в roundUpToAvailableRequired() - см. ниже
...
// То же округление вверх до "в наличии", но для несущих элементов, где заниженная
// толщина недопустима: полоз, торцовый брус дна, подполозная доска. Если расчётная
// толщина превышает даже максимальную из выбранных "в наличии" - в отличие от
// roundUpToAvailable() значение НЕ занижается до складского максимума, а остаётся
// расчётным (по ГОСТ). Предупреждение при этом всё равно выводится - см.
// requiredThicknessNotAvailable ниже.
function roundUpToAvailableRequired(t){
  if(availableThicknesses.length === 0) return t;
  for(const a of availableThicknesses){ if(t<=a) return a; }
  requiredThicknessNotAvailable = true;
  return t;
}
...
function calculate(){
  ...
  requiredThicknessNotAvailable = false;
  ...
  if(solidRigidBase){
    ...
    t9 = roundUpToAvailableRequired(poloz.h); w9 = poloz.w;
  } else {
    ...
    l9 = sel.count; t9 = roundUpToAvailableRequired(sel.h); w9 = sel.w;
    if(sel.massSnapped){
  ...
  const t10=roundUpToAvailableRequired(t10Raw), w10=Math.min(w9, 150), k10=k9Base-400, l10=l9;
  if(k10 < 300){
  ...
  const t11=roundUpToAvailableRequired(endBeam.h), w11=endBeam.w, k11=W, l11=2;
  ...
  if(requiredThicknessNotAvailable){
    warnings.push(`Расчётная толщина полоза, торцового бруса дна или подполозной доски превышает максимальную из «в наличии» (${availableThicknesses[availableThicknesses.length-1]} мм) — для этих несущих элементов занижать толщину нельзя, использовано расчётное значение по ГОСТ (потребуется пиломатериал большей толщины, чем отмечено «в наличии»).`);
  }
